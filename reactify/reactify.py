...
class Reactify(commands.Cog):
    """Automated reactions when trigger words are detected in chat."""
    ...
    async def synergy_delete_data_for_user(self, **kwargs):
        """Nothing to delete."""
        return
    ...
    async def addreact(self, ctx, word, emoji):
        """Adds an auto reaction to a word."""
        guild = ctx.message.guild
        message = ctx.message
        emoji = self.fix_custom_emoji(emoji)
        await self.create_auto_reaction(guild, word, emoji, message)
    ...
    async def delreact(self, ctx, word, emoji):
        """Deletes an auto reaction to a word."""
        guild = ctx.message.guild
        message = ctx.message
        emoji = self.fix_custom_emoji(emoji)
        await self.remove_auto_reaction(guild, word, emoji, message)
    ...
    async def listreact(self, ctx):
        ...
        msg = f"Auto Reactions for {ctx.guild.name}:\n"
    ...
    async def create_auto_reaction(self, guild, word, emoji, message):
        try:
            ...
            if emoji in reactions:
                if word.lower() in reactions[emoji]:
                    await message.channel.send("This auto reaction already exists.")
                    return
    ...
    async def remove_auto_reaction(self, guild, word, emoji, message):
        try:
            # Use the reaction to see if it's valid.
            await message.add_reaction(emoji)
            emoji = str(emoji)
            ...
            if emoji in reactions:
                if word.lower() in reactions[emoji]:
                    ...
                    await self.conf.guild(guild).reactions.set(reactions)
                    await message.channel.send("Removed this auto reaction.")
                else:
                    await message.channel.send("That emoji is not used as a reaction "
                                               "for that word.")
            else:
                await message.channel.send("There are no auto reactions which use "
                                           "this emoji.")
